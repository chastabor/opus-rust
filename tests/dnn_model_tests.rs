...
fn test_pitchdnn_multi_step_vs_c() {
    ...
    let n_steps = 5;
    let nb_if = opus::dnn::pitchdnn::NB_IF_FEATURES;
    let nb_xcorr = opus::dnn::pitchdnn::NB_XCORR_FEATURES;
    ...
    for i in 0..n_steps {
        rust_result = opus::dnn::pitchdnn::compute_pitchdnn(
            &mut state,
...
fn test_rdovae_decode_vs_c() {
    ...
    let dec_arrays = opus::dnn::nnet::weights::parse_weights(&dec_blob).unwrap();
    let dec_model = opus::dnn::dred::rdovae_dec::init_rdovae_dec(&dec_arrays).unwrap();
    ...
    // Rust decode
    let mut dec_state = opus::dnn::dred::rdovae_dec::rdovae_dec_state_init(&dec_model);
    let mut rust_features = vec![0.0f32; 4 * nb_latents * opus::dnn::dred::DRED_NUM_FEATURES];
    opus::dnn::dred::rdovae_dec::dred_rdovae_decode_all(
        &mut dec_state,
        &dec_model,
        &mut rust_features,
        &state,
...
fn test_fargan_single_frame_vs_c() {
    ...
    let arrays = opus::dnn::nnet::weights::parse_weights(&blob).unwrap();
    let model = opus::dnn::fargan::init_fargan(&arrays).unwrap();
    let mut state = opus::dnn::fargan::fargan_state_init(model);
    ...
    opus::dnn::fargan::fargan_cont(&mut state, &cont_pcm, &cont_features);
    let mut rust_pcm = vec![0.0f32; opus::dnn::fargan::FARGAN_FRAME_SIZE];
    opus::dnn::fargan::fargan_synthesize(&mut state, &mut rust_pcm, &features);
...
fn test_fargan_multi_frame_vs_c() {
    ...
    let mut rust_pcm = vec![0.0f32; n_frames * opus::dnn::fargan::FARGAN_FRAME_SIZE];
    for i in 0..n_frames {
        let start = i * opus::dnn::fargan::FARGAN_FRAME_SIZE;
        let end = start + opus::dnn::fargan::FARGAN_FRAME_SIZE;
        opus::dnn::fargan::fargan_synthesize(
            &mut state,
