...
use crate::silk::{MAX_LPC_ORDER, MAX_NB_SUBFR, MAX_SUB_FRAME_LENGTH};
...
pub fn silk_residual_energy_flp(
    ...
    nrgs[1] = gains[1]
        * gains[1]
        * silk_energy_flp(&lpc_res[lpc_res_offset + shift..lpc_res_offset + shift + subfr_length])
            as f32;
    ...
    if nb_subfr == 4 {
        ...
        nrgs[3] = gains[3]
            * gains[3]
            * silk_energy_flp(
                &lpc_res2[lpc_res_offset + shift..lpc_res_offset + shift + subfr_length],
            ) as f32;
    }
}
