...
use crate::dnn::nnet::weights::{WeightError, parse_weights};
...
pub fn load_encoder_dnn(
    ...
    (
        crate::dnn::dred::rdovae_enc::RdovaeEnc,
        crate::dnn::lpcnet::enc::LpcnetEncState,
    ),
    WeightError,
> {
    ...
    let rdovae_enc = crate::dnn::dred::rdovae_enc::init_rdovae_enc(&arrays)?;

    // Init PitchDNN model (needed by LpcnetEncState for feature extraction)
    let pitchdnn_model = crate::dnn::pitchdnn::init_pitchdnn(&arrays)?;
    let pitchdnn_state = crate::dnn::pitchdnn::pitchdnn_state_init(pitchdnn_model);
    let lpcnet_enc_state = crate::dnn::lpcnet::enc::LpcnetEncState::new(pitchdnn_state);
...
pub fn load_decoder_dnn(
    ...
    (
        crate::dnn::dred::rdovae_dec::RdovaeDec,
        crate::dnn::lpcnet::plc::LpcnetPlcState,
        crate::dnn::osce::structs::OsceModel,
    ),
    WeightError,
> {
    let arrays = parse_weights(data).ok_or(WeightError)?;
    ...
    let rdovae_dec = crate::dnn::dred::rdovae_dec::init_rdovae_dec(&arrays)?;

    // Init PLC: PlcModel + FARGAN + PitchDNN -> LpcnetPlcState
    let plc_model = crate::dnn::lpcnet::plc::init_plcmodel(&arrays)?;
    let pitchdnn_model = crate::dnn::pitchdnn::init_pitchdnn(&arrays)?;
    let pitchdnn_state = crate::dnn::pitchdnn::pitchdnn_state_init(pitchdnn_model);
    let enc_state = crate::dnn::lpcnet::enc::LpcnetEncState::new(pitchdnn_state);
    let fargan_model = crate::dnn::fargan::init_fargan(&arrays)?;
    let fargan_state = crate::dnn::fargan::fargan_state_init(fargan_model);
    let plc_state = crate::dnn::lpcnet::plc::lpcnet_plc_init(plc_model, fargan_state, enc_state);

    // Init OSCE models (reuse already-parsed arrays instead of re-parsing blob)
    let mut osce_model = crate::dnn::osce::structs::OsceModel::default();
    crate::dnn::osce::osce_load_models_from_arrays(&mut osce_model, &arrays)?;
