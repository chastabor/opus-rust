use crate::celt::fft::KissFftCpx;

use crate::dnn::freq::*;
use crate::dnn::pitchdnn::*;
...
pub fn compute_frame_features(st: &mut LpcnetEncState, input: &[f32]) {
    ...
    st.if_features[0] = ((1.0 / 64.0)
        * (10.0 * crate::celt::mathops::celt_log10(1e-15 + x_fft[0].r * x_fft[0].r) - 6.0))
        .clamp(-1.0, 1.0);
    for (i, xf) in x_fft.iter().enumerate().take(PITCH_IF_MAX_FREQ).skip(1) {
        ...
        st.if_features[3 * i] = ((1.0 / 64.0)
            * (10.0 * crate::celt::mathops::celt_log10(1e-15 + xf.r * xf.r + xf.i * xf.i) - 6.0))
            .clamp(-1.0, 1.0);
    }
    ...
    for i in 0..NB_BANDS {
        ly[i] = crate::celt::mathops::celt_log10(1e-2 + ex[i]);
    ...
    let buf = &st.exc_buf;
    let ener0: f32 = crate::celt::mathops::celt_inner_prod(&buf[pm..], &buf[pm..], FRAME_SIZE);
    let mut ener1: f64 =
        crate::celt::mathops::celt_inner_prod(&buf[0..], &buf[0..], FRAME_SIZE) as f64;

    let mut xcorr = [0.0f32; PITCH_MAX_PERIOD];
    crate::celt::pitch::celt_pitch_xcorr(
    ...
    let lp = &st.lp_buf;
    let xx = crate::celt::mathops::celt_inner_prod(&lp[pm..], &lp[pm..], FRAME_SIZE);
    let yy = crate::celt::mathops::celt_inner_prod(&lp[pm - pitch..], &lp[pm - pitch..], FRAME_SIZE);
    let xy = crate::celt::mathops::celt_inner_prod(&lp[pm..], &lp[pm - pitch..], FRAME_SIZE);
