use crate::celt::mathops::celt_udiv;
...
// =========================================================================
// Encoder version of the allocation functions
// =========================================================================

/// Compute bit allocation for the encoder, returning the number of coded bands.
/// This is the encoder counterpart of `clt_compute_allocation`.
/// The key difference is that skip/intensity/dual_stereo decisions are WRITTEN
/// to the bitstream rather than READ from it.
///
...
fn interp_bits2pulses_enc(
    ...
    // Skip band decision (encoder version: write decisions)
    let mut coded_bands = end;
    let prev = if prev_coded_bands > 0 {
        prev_coded_bands as usize
    } else {
        end
    };
    loop {
        ...
        if band_bits >= thresh[j].max(alloc_floor + (1 << BITRES)) {
            // Encoder skip decision logic (matching C code)
    ...
    // Code intensity and dual stereo parameters (encoder: write them)
    if intensity_rsv > 0 {
        *intensity = (*intensity).min(coded_bands as i32);
        ec.enc_uint(
    ...
    // Compute fine energy bits (same as decoder)
    let mut bal = 0i32;
    for j in start..coded_bands {
        let n0 = (m.ebands[j + 1] - m.ebands[j]) as i32;
        let n = n0 << lm;
