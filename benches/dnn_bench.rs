...
fn load_blob(name: &str) -> Option<Vec<opus::dnn::nnet::WeightArray>> {
    ...
    opus::dnn::nnet::weights::parse_weights(&data)
}
...
fn bench_model_weights(c: &mut Criterion) {
    ...
    if let Some(arrays) = load_blob("pitchdnn.bin")
        && let Ok(model) = opus::dnn::pitchdnn::init_pitchdnn(&arrays)
    {
        let gru_n = model.gru_1_recurrent.nb_inputs;
        let mut state = opus::dnn::pitchdnn::PitchDnnState {
            model,
            gru_state: vec![0.0f32; gru_n],
            xcorr_mem1: vec![0.0f32; (opus::dnn::pitchdnn::NB_XCORR_FEATURES + 2) * 16],
            xcorr_mem2: vec![0.0f32; (opus::dnn::pitchdnn::NB_XCORR_FEATURES + 2) * 16],
        };
        let mut rng = Rng::new(42);
        let if_features = rng.vec_f32(opus::dnn::pitchdnn::NB_IF_FEATURES);
        let xcorr_features = rng.vec_f32(opus::dnn::pitchdnn::NB_XCORR_FEATURES);
    ...
    if let Some(arrays) = load_blob("fargan.bin")
        && let Ok(model) = opus::dnn::fargan::init_fargan(&arrays)
    {
    ...
    if let Some(arrays) = load_blob("lace.bin") {
        group.bench_function("lace_init", |b| {
            b.iter(|| opus::dnn::osce::lace::init_lace(&arrays).unwrap());
        });
    }
    ...
    if let Ok(data) = std::fs::read(&pitchdnn_path) {
        group.bench_function("parse_weights_pitchdnn", |b| {
            b.iter(|| opus::dnn::nnet::weights::parse_weights(&data).unwrap());
        });
    }
